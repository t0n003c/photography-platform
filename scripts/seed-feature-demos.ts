...
import { BUCKETS, FALLBACK_BUCKET, generateVariant } from "@/src/image/derivatives";
import { generateLqip } from "@/src/image/lqip";
import { Block as BlockSchema, parseBlocks } from "@/src/lib/blocks";
import { getEnv } from "@/src/lib/env";
import { newId } from "@/src/lib/id";
import { getStorage } from "@/src/storage";
import { originalKey, variantKey } from "@/src/storage/keys";
...
const homepageConcepts = [
  {
    slug: "demo-home-editorial-studio",
    title: "Editorial Studio Home",
    subtitle: "Portfolio-led booking homepage",
  },
  {
    slug: "demo-home-wedding-stories",
    title: "Wedding Stories Home",
    subtitle: "Full-service wedding homepage",
  },
  {
    slug: "demo-home-commercial-brand",
    title: "Commercial Brand Home",
    subtitle: "Agency and brand campaign homepage",
  },
  {
    slug: "demo-home-travel-locations",
    title: "Travel Locations Home",
    subtitle: "Map-led destination photography homepage",
  },
  {
    slug: "demo-home-proofing-client",
    title: "Proofing Client Home",
    subtitle: "Private-gallery and trust homepage",
  },
  {
    slug: "demo-home-fine-art-shop",
    title: "Fine Art Shop Home",
    subtitle: "Print sales and gallery homepage",
  },
  {
    slug: "demo-home-senior-portraits",
    title: "Senior Portraits Home",
    subtitle: "Session booking homepage",
  },
  {
    slug: "demo-home-event-coverage",
    title: "Event Coverage Home",
    subtitle: "Documentary events homepage",
  },
  {
    slug: "demo-home-family-newborn",
    title: "Family Newborn Home",
    subtitle: "Soft family session homepage",
  },
  {
    slug: "demo-home-fashion-casting",
    title: "Fashion Casting Home",
    subtitle: "Model, casting, and editorial homepage",
  },
] as const;

const pageSlugs = [
  "demo-features",
  ...homepageConcepts.map((item) => item.slug),
...
function demoPhotoId(index: number) {
  return `${BASE}-photo-${String(index + 1).padStart(3, "0")}`;
}

async function ensureDemoPlaceholderPhotos(ownerId: string) {
  const storage = getStorage();
  const ids = Array.from({ length: DEMO_PHOTO_COUNT }, (_, index) =>
    demoPhotoId(index),
  );

  await db.delete(photoVariant).where(inArray(photoVariant.photoId, ids));

  for (let index = 0; index < DEMO_PHOTO_COUNT; index += 1) {
    const id = ids[index];
    const color = PLACEHOLDER_COLORS[index % PLACEHOLDER_COLORS.length];
    const frame = PLACEHOLDER_FRAMES[index % PLACEHOLDER_FRAMES.length];
    const filename = `feature-demo-color-${String(index + 1).padStart(3, "0")}.jpg`;
    const original = await sharp({
      ...
      .toBuffer();
    const originalStorageKey = originalKey(id, "jpg");

    await storage.put(originalStorageKey, original, {
      contentType: "image/jpeg",
    });

    const lqip = await generateLqip(original);
    const title = `Color Field ${String(index + 1).padStart(2, "0")}`;
    ...
    await db
      ...
      .values({
        ...
        width: frame.width,
        height: frame.height,
        captureDate: new Date(Date.UTC(2026, 0, (index % 28) + 1)),
        dominantColor: color,
        lqip,
        blurhash: null,
        altText: `${title} solid color placeholder`,
        headline: title,
        subhead: "Generic blank color photo",
        caption:
          "Generic solid-color placeholder image generated by the feature demo seed.",
        exif: {
          generated: true,
          color,
          purpose: "feature-demo-placeholder",
        },
    ...
    const buckets = BUCKETS.filter(
      (bucket, bucketIndex) => bucket.width <= frame.width || bucketIndex === 0,
    );
    for (const bucket of buckets) {
      const variant = await generateVariant(original, bucket, "webp");
      const key = variantKey(id, variant.sizeBucket, variant.format);
      await storage.put(key, variant.body, {
        contentType: `image/${variant.format}`,
        cacheControl: "public, max-age=31536000, immutable",
      });
      await db.insert(photoVariant).values({
        id: newId(),
        photoId: id,
    ...
    const fallback =
      buckets.find((bucket) => bucket.name === FALLBACK_BUCKET) ??
      buckets[buckets.length - 1] ??
      buckets[0];
    if (fallback) {
      const variant = await generateVariant(original, fallback, "jpeg");
      const key = variantKey(id, variant.sizeBucket, variant.format);
      await storage.put(key, variant.body, {
        contentType: `image/${variant.format}`,
        cacheControl: "public, max-age=31536000, immutable",
...
async function main() {
  const productRows = await db.execute(
    sql`select count(*)::int as count from product where is_active = true`,
  );
  const productCount = Number(
    (productRows as unknown as Array<{ count: number }>)[0]?.count ?? 0,
  );
  const ownerId = await ensureOwner();
  const photoIds = await ensureDemoPlaceholderPhotos(ownerId);
  ...
  const homepagePricing = (
    ...
    showBillingToggle: index % 3 !== 0,
    theme: (["auto", "dark", "light"] as const)[index % 3],
    showHighlightEffect: index % 2 === 0,
    pricingSliderBackgroundPhotoId: pick(48 + index),
    pricingSliderOverlayOpacity: 0.4,
    pricingSliderHeadingSize: (["small", "reference", "large"] as const)[index % 3],
    pricingSliderEyebrowSize: (["small", "reference", "large"] as const)[
      (index + 1) % 3
    ],
    pricingSliderAutoplay: index % 2 === 0,
    pricingSliderAutoplayMs: 4400 + index * 250,
    pricingSliderTransitionMs: 900,
    castingImageRatio: (["reference", "wide", "square", "portrait"] as const)[
      index % 4
    ],
    plans: pricingPlans,
    ...overrides,
  ...
  const homepageContact = (
    ...
    contactHeroOverlayOpacity: 0.35,
    contactInfoEyebrow: "CONTACT",
    contactInfoHeading: "CONTACT INFO",
    contactInfoIntro:
      "For studio booking, licensing, proofing, or print support, choose the best contact below.",
    ...
    contactImagePhotoIds: group(44 + index * 3, 6),
    contactSideCaption: "Designed for publish-ready photography homepages.",
    ...overrides,
  });
  ...
  const homepageCta = (
    ...
    buttonStyle: "solid" | "pill" | "outline" | "soft" | "link" = "pill",
  ): BlockInput => ({
    id: id(),
    type: "cta",
    headline,
  ...
  {
    const id = idFactory("demo-home-editorial");
    pages.push({
      id: `${BASE}-page-home-editorial-studio`,
      slug: "demo-home-editorial-studio",
      title: "Editorial Studio Homepage",
      type: "landing",
      theme: "auto",
      seoDescription:
        "A publish-ready editorial photography homepage using hero, portfolio, gallery, testimonials, pricing, and contact sections.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...
          photoId: pick(0),
          photoIds: group(0, 5),
          slides: sliderSlides(0, 5, "home-editorial"),
          eyebrow: "EDITORIAL STUDIO",
          headline: "Portraits with quiet direction",
          subhead:
            "A composed homepage for portrait, editorial, and personal brand photography.",
          ctaLabel: "View stories",
          ctaHref: "/galleries/demo-gallery-justified",
          height: "full",
          overlay: "dark",
          effect: "ken-burns",
          headlineFont: "serif",
          headlineSize: "xl",
          fullWidthSliderAccentColor: "#f8f3df",
          fullWidthSliderDimImages: true,
        },
        {
          id: id(),
          type: "heading",
          text: "A polished booking path for image-led clients",
          level: 1,
          align: "center",
          font: "playfair",
          spacing: "normal",
          headingStyle: "tora-modern",
          label: "HOME CONCEPT",
          body: "This homepage combines a full-width slider, portfolio story links, a category-led gallery, testimonials, pricing, and a direct inquiry flow.",
        },
        {
          id: id(),
          type: "portfolioList",
          ...
          body: "Feature recent portrait, campaign, and studio sessions before moving visitors into booking.",
          items: portfolioItems(0, 5, "/galleries/demo-gallery-justified"),
        ...
        homepageGallery(id, {
          ...
          gridType: "tora-justified-showcase",
          limit: 12,
          toraJustifiedUseBackground: false,
          toraJustifiedTitleColor: "#252626",
          toraJustifiedAccentColor: "#9c6b35",
          toraJustifiedDimOnLeadHover: false,
          toraJustifiedShowBlurredSideFill: false,
        }),
        {
          ...
          layout: "glass-stack",
          label: "CLIENT NOTES",
          title: "Calm direction, clear delivery",
          subtitle: "Trust copy for a homepage before the pricing section.",
          glassShowcaseBackground: true,
          glassShowcaseBackgroundColor: "#111827",
          autoplay: true,
          showThumbnails: true,
        ...
        homepagePricing(
          ...
          "tora-pricing-slider",
          "Editorial packages",
          "A slider pricing section for portrait, editorial, and commercial inquiry paths.",
          0,
        ),
        homepageContact(
          id,
          "tora-images-form",
          "Start with a session note",
          "Share the use case, timing, and visual direction. The reply can include availability, package fit, and next steps.",
          0,
        ),
      ],
    });
  ...
  {
    const id = idFactory("demo-home-wedding");
    pages.push({
      id: `${BASE}-page-home-wedding-stories`,
      slug: "demo-home-wedding-stories",
      title: "Wedding Stories Homepage",
      type: "landing",
      theme: "auto",
      seoDescription:
        "A wedding photography homepage with a full hero, detail gallery, info blocks, pricing, FAQ, and contact flow.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...BANNER_DEFAULTS,
          layout: "toramochie-wedding-studio",
          photoId: pick(48),
          photoIds: group(48, 5),
          eyebrow: "WEDDING STORIES",
          headline: "Soft ceremony stories with heirloom detail",
          subhead:
            "A warm wedding homepage built around story, service clarity, and inquiry conversion.",
          ctaLabel: "See wedding details",
          ctaHref: "/categories/demo-wedding-details",
          height: "full",
          overlay: "auto",
          effect: "reveal",
          headlineFont: "serif",
        ...
        },
        {
          ...
          style: "creative",
          eyebrow: "APPROACH",
          title: "A calm plan for the full day",
          text: "From preparation to reception, the page explains how coverage feels organized without becoming heavy.",
          quote: "The best wedding photographs feel present, not staged.",
          photoId: pick(49),
          secondaryPhotoId: pick(50),
          dimPhoto: false,
          creativeTextLayout: "reference",
          creativePhotoSize: "70",
          creativePhotoRatio: "4-5",
          buttonLabel: "Ask about dates",
          buttonHref: "/demo-contact-styles",
        },
        {
          id: id(),
          ...
          body: "A story-led project list for ceremony, portraits, details, and reception coverage.",
          items: portfolioItems(48, 5, "/categories/demo-wedding-details"),
        ...
        homepagePricing(
          ...
          "tora-classic",
          "Wedding coverage",
          "A classic pricing section for day coverage, proofing, retouching, and print-ready delivery.",
          1,
          { showBillingToggle: false, theme: "light" },
        ),
        {
          id: id(),
          type: "faq",
          title: "Wedding planning questions",
          style: "bordered",
          align: "center",
          items: faqItems,
        },
  ...
  {
    const id = idFactory("demo-home-commercial");
    pages.push({
      id: `${BASE}-page-home-commercial-brand`,
      slug: "demo-home-commercial-brand",
      title: "Commercial Brand Homepage",
      type: "landing",
      theme: "auto",
      seoDescription:
        "A commercial photography homepage for agencies and brands with hero, client wall, team, case studies, pricing, and CTA sections.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...
          headline: "Image libraries for launch days",
          subhead:
            "A commercial homepage for agencies, founders, product teams, and editorial campaigns.",
          ctaLabel: "Review services",
          ctaHref: "/demo-pricing-styles",
          height: "full",
          overlay: "dark",
        ...
        },
        {
          ...
          eyebrow: "BEST CASES",
          intro:
            "A client-wall section can introduce brand credibility before portfolio proof.",
          style: "tora-client-wall",
          grayscale: true,
          size: "md",
          spacing: "normal",
          photoIds: group(64, 10),
        },
        {
          id: id(),
          type: "portfolioList",
          ...
          title: "Commercial sets",
          body: "Large campaign teasers route visitors into galleries, services, and inquiry.",
          items: portfolioItems(64, 5, "/categories/demo-commercial-sets"),
        ...
        }),
        {
          ...
          layout: "toraCrew",
          toraCrewEyebrow: "CREW",
          toraCrewShowHiring: true,
          toraCrewHiringTitle: "BOOK THE TEAM",
          ...
          toraCrewHiringLinks: [
            ...
            { id: "editor", title: "EDITOR", subtitle: "DELIVERY", href: "#" },
          ],
          grayscale: true,
          showSocials: true,
          members: teamMembers,
        ...
        homepagePricing(
          ...
          "Package cards with media panels for day rates, brand libraries, and campaign weeks.",
          2,
        ),
        homepageCta(
          id,
          "Build the shot list",
          "Use a compact CTA after services to move agencies toward inquiry.",
          "/demo-contact-styles",
          "solid",
        ),
      ],
  ...
  {
    const id = idFactory("demo-home-travel");
    pages.push({
      id: `${BASE}-page-home-travel-locations`,
      slug: "demo-home-travel-locations",
      title: "Travel Locations Homepage",
      type: "landing",
      theme: "auto",
      seoDescription:
        "A destination photography homepage using location maps, route planning, scroll showcase, location galleries, and inquiry sections.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...BANNER_DEFAULTS,
          layout: "toramochie-full-wall",
          photoId: pick(58),
          photoIds: group(58, 6),
          eyebrow: "LOCATION STORIES",
          headline: "Travel work organized by place",
          subhead:
            "A homepage concept for photographers who sell destination sessions, travel journals, or location-based stories.",
          ctaLabel: "Browse locations",
          ctaHref: "/locations/demo-chicago-studio",
          height: "full",
          overlay: "dark",
          effect: "ken-burns",
          headlineFont: "serif",
        ...
        },
        {
          ...
          type: "locationMap",
          title: "Field routes",
          subtitle: "Show where the work was made and link each marker to a story.",
          locationIds: demoLocations.map((item) => item.id),
          customPins: [
            {
              id: "travel-pin-1",
              title: "Seasonal route",
              subtitle: "Midwest to mountain coverage",
              lat: "41.881",
              lng: "-87.63",
              photoId: pick(58),
              linkLabel: "View Chicago",
              linkHref: "/locations/demo-chicago-studio",
            },
            {
              id: "travel-pin-2",
              title: "Print drop",
              subtitle: "Landscape collection release",
              lat: "39.739",
              lng: "-104.99",
              photoId: pick(60),
              linkLabel: "View shop",
        ...
        },
        {
          ...
          showTitles: true,
          style: "scrollLayouts",
          scrollLayoutsVariant: "bento",
          scrollLayoutsHeading: "Location-led stories",
          scrollLayoutsIntroText:
            "A scroll section can bridge travel copy, location pages, and image sets.",
          scrollLayoutsPhotoCount: 9,
          scrollLayoutsCaption: "Travel route",
          scrollLayoutsUseBackground: true,
          scrollLayoutsBackground: "#f5efe6",
          scrollLayoutsTextColor: "#171717",
        },
        homepageGallery(id, {
          source: "location",
          targetId: demoLocations[2].id,
          gridType: "horizontal-lenis",
          limit: 12,
          backdrop: "neutral",
        }),
        ...
        homepageContact(
          ...
          "minimal",
          "Plan a route",
          "Send the location, season, and story goal so the production plan can start cleanly.",
          3,
        ),
      ],
    });
  ...
  {
    const id = idFactory("demo-home-proofing");
    pages.push({
      id: `${BASE}-page-home-proofing-client`,
      slug: "demo-home-proofing-client",
      title: "Proofing Client Homepage",
      type: "landing",
      theme: "auto",
      seoDescription:
        "A client proofing homepage focused on galleries, favorites, downloads, testimonials, FAQ, and support contact sections.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...
          headline: "Proofing that feels simple",
          subhead:
            "A homepage for studios that want to explain private galleries, favorites, downloads, and ordering.",
          ctaLabel: "Open proofing demo",
          ctaHref: "/demo-proof-team-logos-faq",
          height: "tall",
          overlay: "auto",
          effect: "reveal",
          headlineFont: "serif",
        ...
        customLinkBlock(id(), "link-row", [
          {
            ...
            subtitle: "Browse proofing layouts",
            href: "/galleries/demo-gallery-uniform",
          },
          {
            title: "Private proofing",
            subtitle: "Seed prints a fresh share link",
            href: "/demo-proof-team-logos-faq",
          },
          {
            title: "Client support",
            subtitle: "Contact and delivery notes",
            href: "/demo-contact-styles",
          },
        ]),
        {
          ...
          style: "tabs",
          eyebrow: "PROCESS",
          title: "Review, select, download",
          text: "Use tabs to explain the client flow without making the homepage feel like documentation.",
          photoId: pick(14),
          secondaryPhotoId: pick(18),
          dimPhoto: true,
          buttonLabel: "Ask about proofing",
          buttonHref: "/demo-contact-styles",
          tabs: ["Proof", "Favorite", "Deliver"].map((title, tabIndex) => ({
            id: `proofing-tab-${tabIndex}`,
            title,
            text: "A clear client step with gallery access, selection notes, and delivery expectations.",
            photoId: pick(16 + tabIndex),
            accentPhotoId: pick(20 + tabIndex),
          })),
          accordionItems: [],
        },
        homepageGallery(id, {
          source: "gallery",
          targetId: demoGalleries[2].id,
          gridType: "uniform",
          limit: 12,
          spacing: "tight",
        }),
        {
          ...
          label: "CLIENT NOTES",
          title: "Easy review for real clients",
          subtitle: "Proofing pages need trust, clarity, and a clear next action.",
          gridPanel: true,
          gridColumns: "3",
          autoplay: false,
          showThumbnails: true,
        ...
        homepageContact(
          id,
          "card",
          "Need help with a gallery?",
          "Send the gallery name, deadline, and what you need to review or download.",
          4,
        ),
      ],
    });
  ...
  {
    const id = idFactory("demo-home-fine-art");
    pages.push({
      id: `${BASE}-page-home-fine-art-shop`,
      slug: "demo-home-fine-art-shop",
      title: "Fine Art Shop Homepage",
      type: "landing",
      theme: "auto",
      seoDescription:
        "A fine-art photography homepage for print sales using a minimal hero, gallery showcase, shop, book slider, CTA, and Instagram sections.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...
          photoId: pick(32),
          photoIds: group(32, 5),
          slides: sliderSlides(32, 5, "home-fine-art"),
          eyebrow: "LIMITED PRINTS",
          headline: "Quiet color fields for walls and rooms",
          subhead:
            "A minimal home page for print drops, gallery viewing, and collectors.",
          ctaLabel: "Shop preview",
          ctaHref: "/demo-commerce-conversion",
          height: "full",
          overlay: "none",
          effect: "none",
          headlineFont: "serif",
          headlineSize: "xl",
          minimalSliderAutoplay: true,
          minimalSliderAutoplayMs: 4600,
        },
        homepageGallery(id, {
          source: "category",
          targetId: demoCategories[2].id,
          gridType: "tora-props-catalog",
          limit: 12,
          toraPropsShowBackground: false,
          toraPropsShowCaptions: true,
          toraPropsCaptionColor: "#252626",
        }),
        {
          id: id(),
          type: "shop",
          ...
          body:
            productCount > 0
              ? "Browse print and digital product examples with filtering, prices, and sale labels."
        ...
        },
        {
          id: id(),
          type: "bookSlider",
          title: "Collector lookbook",
          subtitle: "Click or drag pages to browse a print story.",
          coverTitle: "Print Notes",
          coverSubtitle: "A seasonal collection",
          coverPhotoId: pick(34),
          pages: group(35, 5).map((photoId, pageIndex) => ({
            id: `fine-art-book-page-${pageIndex}`,
            photoId,
            imageMode: pageIndex % 2 === 0 ? "editorial" : "full",
            headline: ["Tone", "Paper", "Scale", "Frame", "Room"][pageIndex],
            subhead: "A print-shop page for quiet browsing and collector education.",
            caption: "Fine art demo",
            linkLabel: "Shop",
            linkHref: "/demo-commerce-conversion",
          })),
          size: "standard",
        ...
        { id: id(), type: "instagram", title: "Studio fragments", count: 12 },
        homepageCta(
          id,
          "Collect the next release",
          "Use the homepage to bridge browsing, inquiry, and print sales without a separate landing page.",
          "/demo-commerce-conversion",
          "outline",
        ),
      ],
  ...
  {
    const id = idFactory("demo-home-senior");
    pages.push({
      id: `${BASE}-page-home-senior-portraits`,
      slug: "demo-home-senior-portraits",
      title: "Senior Portraits Homepage",
      type: "landing",
      theme: "auto",
      seoDescription:
        "A senior portrait homepage with booking-oriented hero, carousel, comparison, pricing, testimonials, and contact sections.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...
          headline: "Clean portraits for a milestone year",
          subhead:
            "A session-focused homepage for seniors, families, and school-year campaigns.",
        ...
        },
        {
          ...
          headline: "Three looks, one simple session",
          highlightText: "simple",
          highlightFrom: "#ef476f",
          highlightTo: "#118ab2",
          subtitle:
            "Use a carousel to show outfits, locations, proofing, and delivery in a phone-friendly rhythm.",
          photoIds: group(8, 8),
          autoplay: true,
          autoplayMs: 3600,
          showArrows: true,
          ...
          primaryHref: "/galleries/demo-gallery-masonry",
        ...
        },
        {
          ...
          title: "Retouching preview",
          subtitle:
            "A comparison block can explain proofing, edit style, or crop options.",
          leftPhotoId: pick(10),
          rightPhotoId: pick(11),
          leftLabel: "Proof",
          rightLabel: "Final",
        ...
        homepagePricing(
          ...
          "tora-simple",
          "Portrait sessions",
          "A clear pricing section for mini sessions, full sessions, and extended packages.",
          5,
        ),
        {
          id: id(),
          ...
          layout: "slider",
          label: "CLIENT NOTES",
          title: "Ready for announcements and prints",
          subtitle: "Short trust copy works well near booking CTAs.",
          autoplay: true,
          showThumbnails: true,
          items: testimonialItems,
        },
        homepageContact(
          id,
          "split",
          "Reserve a portrait date",
          "Send preferred dates, location ideas, wardrobe notes, and delivery timing.",
          6,
        ),
      ],
    });
  ...
  {
    const id = idFactory("demo-home-events");
    pages.push({
      id: `${BASE}-page-home-event-coverage`,
      slug: "demo-home-event-coverage",
      title: "Event Coverage Homepage",
      type: "landing",
      theme: "auto",
      seoDescription:
        "An event photography homepage with documentary hero, event gallery, process accordion, team, pricing, and CTA sections.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...
          headline: "Documentary coverage for rooms in motion",
          subhead:
            "A practical homepage for ceremonies, launches, conferences, and private gatherings.",
          ctaLabel: "See event stories",
          ctaHref: "/categories/demo-event-stories",
          height: "full",
          overlay: "dark",
          effect: "reveal",
          headlineFont: "sans",
        ...
        }),
        {
          ...
          style: "accordion",
          eyebrow: "FLOW",
          title: "Coverage that follows the timeline",
          text: "Use accordions for schedule, shot list, delivery, and private gallery notes.",
          photoId: pick(18),
          secondaryPhotoId: pick(19),
          dimPhoto: true,
          buttonLabel: "Send the timeline",
          buttonHref: "/demo-contact-styles",
          tabs: [],
          accordionItems: ["Timeline", "Shot list", "Delivery"].map(
            (title, accordionIndex) => ({
              id: `event-accordion-${accordionIndex}`,
              title,
              text: "A concise production note for event planners and hosts.",
            }),
          ),
        },
        {
        ...
        homepagePricing(
          ...
          "tora-modern",
          "Event coverage",
          "A modern pricing section for hourly, half-day, and full-event coverage.",
          6,
        ),
        homepageCta(
          id,
          "Send the run of show",
          "Move event visitors from galleries into a concrete planning request.",
          "/demo-contact-styles",
          "pill",
        ),
      ],
  ...
  {
    const id = idFactory("demo-home-family");
    pages.push({
      id: `${BASE}-page-home-family-newborn`,
      slug: "demo-home-family-newborn",
      title: "Family Newborn Homepage",
      type: "landing",
      theme: "auto",
      seoDescription:
        "A family and newborn photography homepage with gentle hero, about, gallery, testimonials, FAQ, pricing, and contact sections.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...
          headline: "Simple photographs for growing families",
          subhead:
            "A warm homepage concept for newborn, family, and at-home portrait sessions.",
        ...
        },
        {
          ...
          sectionTitle: "Gentle sessions at home or outside",
          eyebrow: "FAMILY PHOTOGRAPHER",
          headline: "Calm direction for real moments",
          body: "A family homepage can introduce the photographer, explain the session rhythm, and reduce friction before inquiry.",
          quote: "The best family photographs leave room for the day to happen.",
          ctaLabel: "Meet the studio",
          ctaHref: "/demo-about-profile-styles",
          primaryPhotoId: pick(24),
          secondaryPhotoId: pick(25),
          tertiaryPhotoId: pick(26),
          ...
          awardsTitle: "NOTES",
          awardLinks: [],
          collaboratorsTitle: "SESSION SUPPORT",
          collaboratorsText:
            "Wardrobe notes, proofing galleries, and simple print options.",
        ...
        }),
        {
          ...
          label: "FAMILY NOTES",
          title: "Easy sessions, soft delivery",
          subtitle:
            "Testimonials are especially useful for family and newborn homepages.",
          autoplay: true,
          showThumbnails: true,
          items: testimonialItems,
        },
        homepagePricing(
          ...
          "tora-creative",
          "Family sessions",
          "A soft service section for newborn, family, and seasonal portrait packages.",
          7,
          { theme: "light" },
        ),
        {
        ...
        homepageContact(
          ...
          "Share ages, location, timing, and whether you want home, studio, or outdoor coverage.",
          7,
        ),
      ],
    });
  ...
  {
    const id = idFactory("demo-home-fashion");
    pages.push({
      id: `${BASE}-page-home-fashion-casting`,
      slug: "demo-home-fashion-casting",
      title: "Fashion Casting Homepage",
      type: "landing",
      theme: "dark",
      seoDescription:
        "A fashion and casting photography homepage with creative hero, casting profile, models masonry, crew carousel, casting pricing, and contact sections.",
      blocks: [
        {
          id: id(),
          type: "banner",
          ...
          photoId: pick(72),
          photoIds: group(72, 5),
          eyebrow: "CASTING STUDIO",
          headline: "Model tests, lookbooks, and casting support",
          subhead:
            "A high-contrast homepage for fashion, casting, and editorial portfolio production.",
          ctaLabel: "View model work",
          ctaHref: "/demo-portfolio-list-styles",
          height: "full",
          overlay: "dark",
          effect: "css-glitch-1",
          headlineFont: "sans",
        ...
        },
        {
          ...
          type: "about",
          layout: "tora-casting",
          sectionEyebrow: "CASTING",
          sectionTitle: "Model tests with production clarity",
          eyebrow: "PORTFOLIO",
          headline: "Sharp image sets for casting and editorial review",
          body: "A casting homepage can introduce the offer, show range, and make booking direct without burying the portfolio.",
          quote: "Range, expression, and clean delivery matter most.",
          ctaLabel: "Book a test",
          ctaHref: "/demo-contact-styles",
          primaryPhotoId: pick(74),
          secondaryPhotoId: pick(75),
          tertiaryPhotoId: pick(76),
          contactTitle: "CONTACT",
          address: "231 Main Street Chicago, IL",
          phoneLabel: "Ph:",
          phoneNumber: "3122299000",
          ...
          awardsTitle: "CASTING",
          awardLinks: [],
          collaboratorsTitle: "PRODUCTION",
          collaboratorsText:
            "Studio, styling, selects, retouching, and gallery delivery.",
          showContactForm: false,
          contactFormTitle: "CONTACT ME",
          submitLabel: "Send note",
        },
        {
          ...
          eyebrow: "MODEL WORK",
          title: "Casting portfolios",
          body: "A masonry portfolio creates a fast overview before services and contact.",
          items: portfolioItems(72, 6, "/categories/demo-commercial-sets"),
        ...
        homepagePricing(
          ...
          "Casting services",
          "Casting-focused package cards with image ratios for tests, books, and editorial sets.",
          8,
          { theme: "dark", castingImageRatio: "portrait" },
        ),
        homepageContact(
          id,
          "tora-contact-info",
          "Send the brief",
          "Share model count, usage, styling needs, and the review deadline.",
          8,
        ),
      ],
    });
  ...
  const validatedPages = pages.map((item) => ({
    ...item,
    blocks: validateBlocks(item.slug, item.blocks),
  }));
  ...
  for (let index = 0; index < validatedPages.length; index += 1) {
    const item = validatedPages[index];
  ...
  console.log(`[seed-feature-demos] placeholder photos: ${photoIds.length}`);
