...
> describes auth tables, it is documenting the *shape Better Auth provides* so the rest of the
...
rendering. `layout` is a small catalog of layout *types*; `page_config` is the per-surface
