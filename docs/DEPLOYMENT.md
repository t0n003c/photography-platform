...
- **Internal bridge network (`internal`):** `db`, `redis`, `seaweedfs`, `seaweedfs-init`, `worker`, `web`, and `cloudflared` communicate by service name. Only `web` (port `${WEB_PORT:-3000}`) is published for NPM. `seaweedfs` publishes **no host ports by default** — the app and worker reach it over the internal network, so it stays invisible from the host. Storage usage is surfaced on the admin **Dashboard → Storage used** card (no need to open SeaweedFS). To debug the SeaweedFS UIs directly, uncomment the `ports:` block on the `seaweedfs` service in `docker/compose.yaml` (S3 API `8333`, filer/file browser `8888`, master UI `9333`), `docker compose up -d seaweedfs`, then re-comment when done. Firewall those ports off from the public internet — they are never behind the tunnel.
...
- **Cache headers** (`middleware.ts`): all private surfaces (`/admin`, `/login`, `/g/` client galleries, `/api/v1/admin`, `/api/v1/g`, `/api/auth`) are sent `Cache-Control: private, no-store`. Public read APIs (`GET /api/v1/*`, excluding `/api/v1/media`) are edge-cacheable with `public, s-maxage=60, stale-while-revalidate=300`. The middleware also sets HSTS (prod HTTPS only), an enforced nonce-based CSP with reporting, and the standard security headers.
...
| Storage usage                  | Admin **Dashboard → Storage used** card (derived from the DB; no SeaweedFS access needed)                                                                                                                                                         |
