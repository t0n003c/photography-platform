...
- **OnScrollLayoutFormations port:** Pages tab → Scroll Showcase now has
  ...
  Follow-up: Store fulfillment basics are active. Migration
  `0020_thankful_hemingway.sql` adds fulfillment status, carrier/tracking fields,
  milestone timestamps, and private fulfillment notes to `order`. `/admin/store`
  ...
  Follow-up: Provider-side live Stripe refunds are active. Migration
  `0022_cheerful_lord_tyger.sql` adds `order_refund.provider_error` so failed provider
  attempts remain auditable. Admin -> Store refund actions now offer "Refund through
  Stripe" for Stripe-paid invoices with a stored PaymentIntent; manual records remain
  available for outside refunds. `StripePaymentProvider.createRefund` calls
  `/v1/refunds` with the local refund id as the idempotency key, stores the Stripe
  refund id/reference/status, reserves pending refunds from the refundable balance, and
  ...
  changes over 820ms like the reference's Isotope/Justified Gallery reflow. It keeps the source-like
  responsive gutters, uppercase Josefin-style gold names, same-row title bars, 1s image zoom hover,
  theme-aware default panel colors (the original dark default hexes are treated as light/dark
  theme tokens rather than forced dark colors), theme-aware no-background image/title surfaces,
  mobile single column, and reduced-motion-safe transitions.
